...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (update_experience($id, $data)) {
        $message = 'Experience berhasil diperbarui!';
        $experience = get_experience_by_id($id);
    } else {
        $message = 'Gagal memperbarui experience entry.';
    }
}
?>
<!DOCTYPE html>
...
<head>
    ...
    <title>Edit Experience - Admin Portfolio</title>
    <style>
        ...
        <div class="section">
            ...
            <form method="POST">
                <div class="form-group">
                    ...
                    <input type="text" id="role" name="role" value="<?= htmlspecialchars($experience['role']) ?>" required>
                </div>
                <div class="form-row" style="display:grid;grid-template-columns:1fr 1fr;gap:20px;">
                    <div class="form-group">
                        <label for="company">Company *</label>
                        <input type="text" id="company" name="company" value="<?= htmlspecialchars($experience['company']) ?>" required>
                    </div>
                    <div class="form-group">
                        <label for="location">Location</label>
                        <input type="text" id="location" name="location" value="<?= htmlspecialchars($experience['location']) ?>">
                    </div>
                </div>
                <div class="form-row" style="display:grid;grid-template-columns:1fr 1fr;gap:20px;">
                    <div class="form-group">
                        <label for="start_date">Start Date</label>
                        <input type="text" id="start_date" name="start_date" value="<?= htmlspecialchars($experience['start_date']) ?>">
                    </div>
                    <div class="form-group">
                        <label for="end_date">End Date</label>
                        <input type="text" id="end_date" name="end_date" value="<?= htmlspecialchars($experience['end_date']) ?>">
                    </div>
                </div>
                <div class="form-group">
                    <label for="details">Details</label>
                    <textarea id="details" name="details"><?= htmlspecialchars($experience['details']) ?></textarea>
                </div>
                <button type="submit" class="btn">Update Experience</button>
                <a href="experience.php" class="btn btn-back">← Kembali</a>
            </form>
        </div>
    </div>
</body>
