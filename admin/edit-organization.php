...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (update_organization($id, $data)) {
        $message = 'Organization berhasil diperbarui!';
        $organization = get_organization_by_id($id);
    } else {
        $message = 'Gagal memperbarui organization entry.';
    }
}
?>
<!DOCTYPE html>
...
<head>
    ...
    <title>Edit Organization - Admin Portfolio</title>
    <style>
        ...
        <div class="section">
            ...
            <form method="POST">
                <div class="form-group">
                    ...
                    <input type="text" id="role" name="role" value="<?= htmlspecialchars($organization['role']) ?>" required>
                </div>
                <div class="form-group">
                    <label for="organization">Organization *</label>
                    <input type="text" id="organization" name="organization" value="<?= htmlspecialchars($organization['organization']) ?>" required>
                </div>
                <div class="form-row" style="display:grid;grid-template-columns:1fr 1fr;gap:20px;">
                    <div class="form-group">
                        <label for="start_date">Start Date</label>
                        <input type="text" id="start_date" name="start_date" value="<?= htmlspecialchars($organization['start_date']) ?>">
                    </div>
                    <div class="form-group">
                        <label for="end_date">End Date</label>
                        <input type="text" id="end_date" name="end_date" value="<?= htmlspecialchars($organization['end_date']) ?>">
                    </div>
                </div>
                <div class="form-group">
                    <label for="description">Description</label>
                    <textarea id="description" name="description"><?= htmlspecialchars($organization['description']) ?></textarea>
                </div>
                <button type="submit" class="btn">Update Organization</button>
                <a href="organizations.php" class="btn btn-back">← Kembali</a>
            </form>
        </div>
    </div>
</body>
