...
if (isset($_GET['delete']) && is_numeric($_GET['delete'])) {
    if (delete_certification($_GET['delete'])) {
        $message = 'Certification entry berhasil dihapus!';
        $certifications = get_certifications();
    }
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['add_certification'])) {
    ...
    if (add_certification($data)) {
        $message = 'Certification berhasil ditambahkan!';
        $certifications = get_certifications();
        $_POST = [];
    } else {
        $message = 'Gagal menambahkan certification entry.';
    }
}
?>
<!DOCTYPE html>
...
<head>
    ...
    <title>Kelola Certifications - Admin Portfolio</title>
    <style>
            ...
            <form method="POST">
                ...
                <div class="form-row" style="display:grid;grid-template-columns:1fr 1fr;gap:20px;">
                    <div class="form-group">
                        ...
                        <input type="text" id="issuer" name="issuer" value="<?= htmlspecialchars($_POST['issuer'] ?? '') ?>">
                    </div>
                    <div class="form-group">
                        <label for="issue_date">Issue Date</label>
                        <input type="text" id="issue_date" name="issue_date" value="<?= htmlspecialchars($_POST['issue_date'] ?? '') ?>" placeholder="e.g. 2024">
                    </div>
                </div>
                <div class="form-group">
                ...
                <button type="submit" class="btn">Simpan Certification</button>
            </form>
        </div>

        <div class="section">
            <h2>🎖️ Certification List</h2>
                    ...
                    <tbody>
                        <?php foreach ($certifications as $item): ?>
                            <tr>
                                ...
                                <td><?= htmlspecialchars($item['issuer']) ?></td>
                                <td><?= htmlspecialchars($item['issue_date']) ?></td>
                                <td><?= nl2br(htmlspecialchars($item['description'])) ?></td>
                                <td>
                                    <div class="actions">
                                        <a href="edit-certification.php?id=<?= $item['id'] ?>" class="btn">Edit</a>
                                        <a href="certifications.php?delete=<?= $item['id'] ?>" class="btn btn-delete" onclick="return confirm('Yakin menghapus entry ini?')">Hapus</a>
                                    </div>
                                </td>
                            </tr>
                        <?php endforeach; ?>
            ...
            <?php else: ?>
                <div class="empty">Belum ada certification entry.</div>
            <?php endif; ?>
        </div>
    </div>
</body>
